...
export const loginUser = functions.https.onCall(async (data) => {
  ...
  const u = snap.data() as { hasPin: boolean; pinHash?: string | null };
  if (u.hasPin) {
    if (!PIN_RE.test(pin))
      throw new functions.https.HttpsError("invalid-argument","Нужен корректный PIN.");
    const ok = await bcrypt.compare(pin, u.pinHash || "");
    if (!ok) throw new functions.https.HttpsError("permission-denied","Неверный PIN.");
  }

  try { await admin.auth().getUser(nick); }
  catch { await admin.auth().createUser({ uid: nick, displayName: nick }); }
